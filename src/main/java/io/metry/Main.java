...
public class Main {
            ...
            while (squareNodeIterator.hasNext()) {
                ...
                switch (index) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        if(jsonNode.textValue().length() != 1 || !lettersWithValue.containsKey(jsonNode.textValue().charAt(0)))
                            throw new IllegalArgumentException("Invalid letters found in board.json");
                        letter = jsonNode.textValue().charAt(0);
                        index++;
                        break;
                    case 3:
                        jsonNode.elements().forEachRemaining(
                                playersNode -> players.add(playersNode.textValue()));
                        index++;
                        break;
                    default:
                        break;
    ...
    private static Map<Character, Integer> buildLettersMap(String lettersFilePath) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            jsonNode.get("letters").elements().forEachRemaining(letterNode -> {
                if (letterNode.textValue().length() != 1)
                    throw new IllegalArgumentException("Invalid letters found in letters.json");
                letters.put(letterNode.textValue().charAt(0), value);
            });
        }
        return letters;
    }
